...
async function getUsers(user_id: number, user_city: string, user_age: number) {

    const res = await fetch(`${BackEnd_URL}/users/best_results/${user_id}/${user_city}/${user_age}`, {
        headers: {
            'ngrok-skip-browser-warning': 'true' // Встановлюємо заголовок для уникнення повідомлення про браузер
        }
    })
...
export default async function UsersPage({params}: any) {
    ...
    const usersData = await getUsers(userData.data.telegram_id, userData.data.city, userData.data.age)
